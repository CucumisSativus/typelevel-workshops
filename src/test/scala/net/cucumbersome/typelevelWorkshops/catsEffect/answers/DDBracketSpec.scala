package net.cucumbersome.typelevelWorkshops.catsEffect.answers

import java.util.UUID

import cats.effect.IO
import net.cucumbersome.typelevelWorkshops.catsEffect.answers.DDBracketSpec.ConnectionPool
...
class DDBracketSpec extends BaseTest{
  ...
  def connectToServiceAndRead(pool: ConnectionPool): IO[String] = {
    IO(pool.getConnection).bracket(c => IO(c.read))(c => IO(c.close))
  }
}
